...
def _build_invoice_information(company, address_1, address_2, phone):
    table_001 = Table(number_of_rows=4, number_of_columns=2, horizontal_alignment=Alignment.RIGHT,
                      column_widths=[Decimal(1), Decimal(6)],)
    # 1
    symbol = ["✁", "✂", "✃", "✄", "☎", "✆", "✇", "✈", "✉", "☛", "☞", "✌", "✍", "✎", "✏", "✐", "✑", "✒", "✓", "✔", "✕", "✖", "✗", "✘", "✙", "✚", "✛", "✜", "✝", "✞", "✟", "✠", "✡", "✢", "✣", "✤", "✥", "✦", "✧", "★", "✩", "✪", "✫", "✬", "✭", "✮", "✯", "✰", "✱", "✲", "✳", "✴", "✵", "✶", "✷", "✸", "✹", "✺", "✻", "✼", "✽", "✾", "✿", "❀", "❁", "❂", "❃", "❄", "❅", "❆", "❇", "❈", "❉", "❊", "❋", "●", "❍", "■", "❏", "❐", "❑", "❒", "▲", "▼", "◆", "❖", "◗", "❘", "❙", "❚"]
    companySymbol = symbol[random.randint(0, len(symbol))]
    table_001.add(TableCell(
        Paragraph(
            companySymbol,
            font="Zapfdingbats",
            font_size=72,
            font_color=HexColor(dark)
            # width=Decimal(128),
            # height=Decimal(128),
        ), row_span=4)
    )
    table_001.add(
        Paragraph(
            company, font=company_font, font_color=HexColor(dark), font_size=16
        )
    )
...
def _build_billing_and_shipping_information(company, address_1, address_2, phone, invoice_num):
    table_001 = Table(number_of_rows=5, number_of_columns=3,column_widths=[Decimal(6), Decimal(1), Decimal(1)],)
    table_001.add(
        Paragraph(
            "Bill to:",
            font_color=HexColor(dark),
    ...
    table_001.add(
        Paragraph(
            "Invoice", font="Helvetica-Bold", font_color=HexColor(dark), horizontal_alignment=Alignment.RIGHT
        )
    )
    ...
    table_001.add(
    Paragraph("Date", font="Helvetica-Bold", font_color=HexColor(dark),
    ...
    table_001.add(
        Paragraph(
            "Due Date", font="Helvetica-Bold", font_color=HexColor(dark), horizontal_alignment=Alignment.RIGHT
        )
    )
...
def _build_itemized_description_table(products: typing.List[Product] = []):
    ...
    """
    table_001 = Table(number_of_rows=15, number_of_columns=4, column_widths=[
                      Decimal(5), Decimal(1), Decimal(2), Decimal(2)], border_top=True, border_color=HexColor(dark))
    for h in ["Item", "Quantity", "Rate", "Line Total"]:
        table_001.add(
            TableCell(
                Paragraph(h, font_color=HexColor(dark)),
    ...
    table_001.add(
        TableCell(
            Paragraph(
                "Subtotal",
                font="Helvetica-Bold",
                horizontal_alignment=Alignment.RIGHT, font_color=HexColor(dark)
            ),
            col_span=3,
        )
    )
    table_001.add(
        TableCell(Paragraph(format_price(subtotal),
                  horizontal_alignment=Alignment.RIGHT))
    )
    ...
    table_001.add(
        TableCell(
            Paragraph(
                "Taxes", font="Helvetica-Bold", horizontal_alignment=Alignment.RIGHT, font_color=HexColor(dark)
            ),
            col_span=3,
        )
    )
    ...
    table_001.add(
        TableCell(
            Paragraph(
                "Total", font="Helvetica-Bold", horizontal_alignment=Alignment.RIGHT, font_color=HexColor(dark)
            ),
            col_span=3,
        )
    )
    ...
    table_001.add(
        TableCell(
            Paragraph(
                "Amount Paid", font="Helvetica-Bold", horizontal_alignment=Alignment.RIGHT, font_color=HexColor(dark)
            ),
            col_span=3,
        )
    )
...
def generate():
    ...
    page_layout.add(_build_invoice_information(
        company, address_1, address_2, phone))
    ...
    page_layout.add(_build_billing_and_shipping_information(
        "Google", "1600 Amphitheatre Pkwy", "Mt View, CA 94043, USA", "123-456-7890", invoice_num))
    ...
    for _ in range(0, num_products):
        ...
        products.append(Product(name, random.randint(
            1, 50), random.randint(10, 10000)))

    page_layout.add(_build_itemized_description_table(products))

    page_layout.add(
        Paragraph("Note: This is a test order. No actual transaction took place."))
...
if __name__ == "__main__":
    for _ in range(0, 10):
        ...
        {
            "company_font": "Helvetica-Bold-Oblique",
            "colors":  {
                "grey_1": "e5e5ed",
                "grey_2": "4c4c85",
                "dark": "004EFF",
        ...
        company_font = styles[style]['company_font']
        grey_1 = styles[style]['colors']['grey_1']
        grey_2 = styles[style]['colors']['grey_2']
        dark = styles[style]['colors']['dark']
        light = styles[style]['colors']['light']
        alternate = False #styles[style]['alternate']
