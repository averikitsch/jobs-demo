...
def _build_invoice_information(company, address_1, address_2, phone, invoice_num):
    back = light if alternate else grey_1
    table_001 = Table(number_of_rows=4, number_of_columns=3,
                      column_widths=[Decimal(5), Decimal(1), Decimal(1)],background_color=HexColor(back))

    # 1
    print(company_font)
    table_001.add(
        Paragraph(
            company, font=company_font, font_color=HexColor(dark), font_size=16, horizontal_alignment=Alignment.LEFT
        )
    )
    table_001.add(
        Paragraph(
...
def _build_itemized_description_table(products: typing.List[Product] = []):
    ...
    table_001 = Table(number_of_rows=15, number_of_columns=4, column_widths=[
                      Decimal(3), Decimal(1), Decimal(2), Decimal(2)],)
    for h in ["Item", "Quantity", "Rate", "Amount"]:
        table_001.add(
            TableCell(
                Paragraph(h, font_color=HexColor(grey_1)),
                background_color=HexColor(dark),
            )
        )
    if alternate:
        odd_color = HexColor(grey_1)
        even_color = HexColor(light)
    else:
        odd_color = HexColor(light)
        even_color = HexColor(light)
...
def generate():
    ...
    page_layout.add(_build_invoice_information(
        company, address_1, address_2, phone, invoice_num))
    ...
    page_layout.add(_build_billing_and_shipping_information(
        "Google", "1600 Amphitheatre Pkwy", "Mt View, CA 94043, USA", "123-456-7890"))
...
if __name__ == "__main__":
    for _ in range(0, 10):
        ...
        alternate = styles[style]['alternate']
